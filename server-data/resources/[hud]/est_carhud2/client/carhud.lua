                ...
                if not seatbeltIsOn then
                    ...
                    if(beltWarningSet == false) then
                        if(currSpeed > 1 or currSpeed < -1) then
                            beltWarningSet = true
                            TriggerEvent("pNotify:SendNotification", {layout = 'topLeft', theme = 'gta', type = 'error', text1 = 'Gurt', text2 = 'Nicht Angeschnallt!'})
			...
			if IsControlJustReleased(0,  Keys['K']) then
				...
				if(PedVehIsCar)then
					...
					if seatbeltIsOn then
						...
						TriggerEvent("pNotify:SendNotification", {layout = 'topLeft', theme = 'gta', type = 'success', text1 = 'Gurt', text2 = 'Angeschnallt!'})
					else
						TriggerEvent("pNotify:SendNotification", {layout = 'topLeft', theme = 'gta', type = 'error', text1 = 'Gurt', text2 = 'Abgeschnallt!'})
						beltWarningSet = false
						if(autopilotActive)then
							DeactivateAutopilot()
						end
