			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
					...
					if Config.MaxInService == -1 then
						ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 182.0, function(vehicle)
							local playerPed = PlayerPedId()
							TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
						end)
					else
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'bennys_craft', {
			...
			if data.current.value == 'blow_pipe' then
				TriggerServerEvent('esx_bennys:startCraft')
			elseif data.current.value == 'fix_kit' then
				TriggerServerEvent('esx_bennys:startCraft2')
			elseif data.current.value == 'caro_kit' then
				TriggerServerEvent('esx_bennys:startCraft3')
			end
		end, function(data, menu)
			menu.close()
...
AddEventHandler('esx_bennys:hasExitedMarker', function(zone)
	if zone =='VehicleDelivery' then
		NPCTargetDeleterZone = false
	elseif zone == 'Craft' then
		TriggerServerEvent('esx_bennys:stopCraft')
		TriggerServerEvent('esx_bennys:stopCraft2')
		TriggerServerEvent('esx_bennys:stopCraft3')
	elseif zone == 'Garage' then
		TriggerServerEvent('esx_bennys:stopHarvest')
		TriggerServerEvent('esx_bennys:stopHarvest2')
		TriggerServerEvent('esx_bennys:stopHarvest3')
	end
