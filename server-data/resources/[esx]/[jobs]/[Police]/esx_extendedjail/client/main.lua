...
exports('OpenJailMenu', function(str)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'jail_menu', {
		...
		if JailTime ~= nil then
			...
			ESX.ShowNotification('Ungültige Zeit')
		end
...
function hacksystem(success)
	if success then
		TriggerServerEvent('esx_extendedjail:removeitem')
		...
		if cb then
			for i=1, #players, 1 do
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'esx_extendedjail', {
					title    = 'Gefängnisdatenbank', align    = 'bottom-right',
					elements = elements
				}, function(data, menu)
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'esx_extendedjail_confirm', {
						title    = 'Bist du sicher?', align    = 'bottom-right',
						elements = {
							{label = ('Nein'),  value = 'no'},
							{label = ('ja'), value = 'yes'}
						}
...
function Animation()
	...
	SetEntityCoords(ped, 1834.5625, 2570.8054199219, 44.891887664795, 1, 0, 0, 1)
