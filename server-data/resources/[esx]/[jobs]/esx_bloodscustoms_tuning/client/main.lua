...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('esx_bloodscustoms_tuning:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_bloodscustoms_tuning:installMod')
AddEventHandler('esx_bloodscustoms_tuning:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_bloodscustoms_tuning:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_bloodscustoms_tuning:cancelInstallMod')
AddEventHandler('esx_bloodscustoms_tuning:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_bloodscustoms_tuning:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('esx_bloodscustoms_tuning:buyMod', price)
		...
		menu.close()
		TriggerEvent('esx_bloodscustoms_tuning:cancelInstallMod')
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'mechanic' or PlayerData.job.name == 'bcustoms') or not Config.IsMechanicJobOnly then
