...
RegisterServerEvent('myIllegals:getJobXP')
AddEventHandler('myIllegals:getJobXP', function()
    ...
    for k, v in pairs(playerXP) do
        if v.steamID == steamID then
            TriggerClientEvent('myIllegals:receiveJobXP', source, v.xp)
        end
    end

end)

RegisterServerEvent('myIllegals:updateIdentifier')
AddEventHandler('myIllegals:updateIdentifier', function(src, oldidentifier, newidentifier)
...
RegisterServerEvent('myIllegals:collectItems')
AddEventHandler('myIllegals:collectItems', function(item, label, amount, xptype)
    ...
    if Config.useIntegratedWeightSystem then
        ...
        if weight + amount * Config.ItemLimits[item] <= Config.MaxCarry then
            ...
            TriggerClientEvent('myIllegals:msg', source, '~g~' .. amount .. 'x ' .. label .. ' ~s~' .. Translation[Config.Locale]['collected'] .. ' (~g~' .. weight + amount * Config.ItemLimits[item] .. '~s~/~y~' .. Config.MaxCarry .. '~s~)')
            if xptype ~= nil then
                addJobXP(source, xptype, amount)
            end
        else
            TriggerClientEvent('myIllegals:msg', source, Translation[Config.Locale]['too_heavy'])
        end

    elseif Config.useESXWeightSystem then
        if xPlayer.canCarryItem(item, amount) then
            xPlayer.addInventoryItem(item, amount)
            TriggerClientEvent('myIllegals:msg', source, '~g~' .. amount .. 'x ' .. label .. ' ~s~' .. Translation[Config.Locale]['collected'])
            if xptype ~= nil then
                addJobXP(source, xptype, amount)
            end
        else
    ...
    --[[else
        ...
        TriggerClientEvent('myIllegals:msg', source, '~y~Das Gebiet ist erschöpft! Komme später wieder, um mehr ~w~' .. label .. ' ~y~zu erhalten.')
...
RegisterServerEvent('myIllegals:craftItem')
AddEventHandler('myIllegals:craftItem', function(itemData)
    ...
    for k, v in pairs(itemData.reqItems) do
        ...
        if itemCount < v.amount then
            TriggerClientEvent('myIllegals:msg', _source, Translation[Config.Locale]['not_enough_items'] .. v.label)
            return
        ...
        if k == #itemData.reqItems then
            TriggerClientEvent('myIllegals:startCrafting', _source, itemData, craftAmount)
        end
...
RegisterServerEvent('myIllegals:finishedCrafting')
AddEventHandler('myIllegals:finishedCrafting', function(itemData, craftAmount)
    ...
    for k, v in pairs(itemData.reqItems) do
        ...
        if k == #itemData.reqItems then
            ...
            TriggerClientEvent('myIllegals:msg', _source, Translation[Config.Locale]['craftting_finished'] .. itemData.newitemamount * craftAmount .. 'x ' .. itemData.newitem.label .. Translation[Config.Locale]['craftting_finished2'])
...
RegisterServerEvent('myIllegals:sellItems')
AddEventHandler('myIllegals:sellItems', function(item, amount, price)
    ...
    if itemCount > 0 and itemCount >= amount then
        ...
        TriggerClientEvent('myIllegals:msg', _source, Translation[Config.Locale]['items_sold_for'] .. price * amount .. '$')

    else
        TriggerClientEvent('myIllegals:msg', _source, Translation[Config.Locale]['you_only_have_items'] .. itemCount .. Translation[Config.Locale]['you_only_have_items2'])
    end

end)

RegisterServerEvent('myIllegals:startDelivery')
AddEventHandler('myIllegals:startDelivery', function(deliveryData)
    ...
    for j, assignment in pairs(shopContent) do
        if assignment.id == deliveryData.id then
            if assignment.state ~= 0 then
                TriggerClientEvent('myIllegals:msg', _source, Translation[Config.Locale]['mission_in_process'])
                return
            else
                for k, v in pairs(deliveryData.requires) do
                    ...
                    if k == #deliveryData.requires then
                        TriggerClientEvent('myIllegals:startDelivery_cl', _source, deliveryData)
                        assignment.state = 1
        ...
        elseif j == #shopContent then
            TriggerClientEvent('myIllegals:msg', _source, Translation[Config.Locale]['mission_not_available'])
            return
        end
    end
...
RegisterServerEvent('myIllegals:abortMission')
AddEventHandler('myIllegals:abortMission', function(id)
...
RegisterServerEvent('myIllegals:finishDelivery')
AddEventHandler('myIllegals:finishDelivery', function(deliveryData)
    ...
    for k, v in pairs(deliveryData.requires) do
        ...
        if itemCount < v.amount then
            TriggerClientEvent('myIllegals:msg', _source, Translation[Config.Locale]['not_enough_items'] .. v.label)
            TriggerEvent('myIllegals:abortMission', deliveryData.id)
            return
        end
        ...
        if k == #deliveryData.requires then
            ...
            TriggerClientEvent('myIllegals:msg', _source, Translation[Config.Locale]['mission_success'] .. deliveryData.label .. Translation[Config.Locale]['mission_success2'] .. deliveryData.price .. Translation[Config.Locale]['mission_success3'])
    ...
    if #playerXP > 0 then
		...
		for k, v in pairs(playerXP) do
            if v.steamID == steamID then
				...
				TriggerClientEvent('myIllegals:receiveJobXP', _source, v.xp)
                break
            ...
            elseif k == #playerXP then
                ...
                TriggerClientEvent('myIllegals:receiveJobXP', _source, xp)
                return
...
RegisterServerEvent('myIllegals:setRandomAssignments')
AddEventHandler('myIllegals:setRandomAssignments', function()
...
RegisterServerEvent('myIllegals:getShopContent')
AddEventHandler('myIllegals:getShopContent', function()

    TriggerClientEvent('myIllegals:receiveShopContent', source, shopContent)
