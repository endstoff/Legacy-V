...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('esx_motorradshop: ^1WARNING^7 plate character count reached, %s/8 characters.'):format(char))
	end
end)
...
MySQL.ready(function()
	...
	TriggerClientEvent('esx_motorradshop:sendCategories', -1, Categories)
	TriggerClientEvent('esx_motorradshop:sendVehicles', -1, Vehicles)
end)

RegisterServerEvent('esx_motorradshop:setVehicleOwned')
AddEventHandler('esx_motorradshop:setVehicleOwned', function (vehicleProps)
...
RegisterServerEvent('esx_motorradshop:setVehicleOwnedPlayerId')
AddEventHandler('esx_motorradshop:setVehicleOwnedPlayerId', function (playerId, vehicleProps)
...
RegisterServerEvent('esx_motorradshop:setVehicleOwnedSociety')
AddEventHandler('esx_motorradshop:setVehicleOwnedSociety', function (society, vehicleProps)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_motorradshop:sellVehicle')
AddEventHandler('esx_motorradshop:sellVehicle', function (vehicle)
...
RegisterServerEvent('esx_motorradshop:addToList')
AddEventHandler('esx_motorradshop:addToList', function(target, model, plate)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(target)
	local dateNow = os.date('%Y-%m-%d %H:%M')

	if xPlayer.job.name ~= 'cardealer' then
		print(('esx_motorradshop: %s attempted to add a sold vehicle to list!'):format(xPlayer.identifier))
		return
	end
...
ESX.RegisterServerCallback('esx_motorradshop:getSoldVehicles', function (source, cb)
...
RegisterServerEvent('esx_motorradshop:rentVehicle')
AddEventHandler('esx_motorradshop:rentVehicle', function (vehicle, plate, playerName, basePrice, rentPrice, target)
...
RegisterServerEvent('esx_motorradshop:getStockItem')
AddEventHandler('esx_motorradshop:getStockItem', function (itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_motorradshop:putStockItems')
AddEventHandler('esx_motorradshop:putStockItems', function (itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('esx_motorradshop:getCategories', function (source, cb)
	cb(Categories)
end)

ESX.RegisterServerCallback('esx_motorradshop:getVehicles', function (source, cb)
	cb(Vehicles)
end)

ESX.RegisterServerCallback('esx_motorradshop:buyVehicle', function (source, cb, vehicleModel)
...
ESX.RegisterServerCallback('esx_motorradshop:buyVehicleSociety', function (source, cb, society, vehicleModel)
	local vehicleData = nil
...
ESX.RegisterServerCallback('esx_motorradshop:getCommercialVehicles', function (source, cb)
...
RegisterServerEvent('esx_motorradshop:returnProvider')
AddEventHandler('esx_motorradshop:returnProvider', function(vehicleModel)
	local _source = source
	...
	MySQL.Async.fetchAll('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			print(('esx_motorradshop: %s attempted selling an invalid vehicle!'):format(GetPlayerIdentifiers(_source)[1]))
		end

	end)
end)

ESX.RegisterServerCallback('esx_motorradshop:getRentedVehicles', function (source, cb)
...
ESX.RegisterServerCallback('esx_motorradshop:giveBackVehicle', function (source, cb, plate)
...
ESX.RegisterServerCallback('esx_motorradshop:resellVehicle', function (source, cb, plate, model)
	local resellPrice = 0
	...
	if resellPrice == 0 then
		print(('esx_motorradshop: %s attempted to sell an unknown vehicle!'):format(GetPlayerIdentifiers(source)[1]))
		cb(false)
	else
		MySQL.Async.fetchAll('SELECT * FROM rented_vehicles WHERE plate = @plate', {
			['@plate'] = plate
					...
					if result[1] then -- does the owner match?
						...
						if vehicle.model == model then
							if vehicle.plate == plate then
								...
								print(('esx_motorradshop: %s attempted to sell an vehicle with plate mismatch!'):format(xPlayer.identifier))
								cb(false)
							end
						else
							print(('esx_motorradshop: %s attempted to sell an vehicle with model mismatch!'):format(xPlayer.identifier))
							cb(false)
						end
					else
...
ESX.RegisterServerCallback('esx_motorradshop:getStockItems', function (source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		cb(inventory.items)
	end)
end)

ESX.RegisterServerCallback('esx_motorradshop:getPlayerInventory', function (source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_motorradshop:isPlateTaken', function (source, cb, plate)
...
RegisterServerEvent('esx_motorradshop:setJobVehicleState')
AddEventHandler('esx_motorradshop:setJobVehicleState', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('esx_motorradshop: %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)
