...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('esx_nutzfahrzeugshop: ^1WARNING^7 plate character count reached, %s/8 characters.'):format(char))
	end
end)
...
MySQL.ready(function()
	Categories     = MySQL.Sync.fetchAll('SELECT * FROM nutzfahrzeuge_categories')
	local vehicles = MySQL.Sync.fetchAll('SELECT * FROM nutzfahrzeuge')
	...
	TriggerClientEvent('esx_nutzfahrzeugshop:sendCategories', -1, Categories)
	TriggerClientEvent('esx_nutzfahrzeugshop:sendVehicles', -1, Vehicles)
end)

RegisterServerEvent('esx_nutzfahrzeugshop:setVehicleOwned')
AddEventHandler('esx_nutzfahrzeugshop:setVehicleOwned', function (vehicleProps)
...
RegisterServerEvent('esx_nutzfahrzeugshop:setVehicleOwnedPlayerId')
AddEventHandler('esx_nutzfahrzeugshop:setVehicleOwnedPlayerId', function (playerId, vehicleProps)
...
RegisterServerEvent('esx_nutzfahrzeugshop:setVehicleOwnedSociety')
AddEventHandler('esx_nutzfahrzeugshop:setVehicleOwnedSociety', function (society, vehicleProps)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_nutzfahrzeugshop:sellVehicle')
AddEventHandler('esx_nutzfahrzeugshop:sellVehicle', function (vehicle)
...
RegisterServerEvent('esx_nutzfahrzeugshop:addToList')
AddEventHandler('esx_nutzfahrzeugshop:addToList', function(target, model, plate)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(target)
	local dateNow = os.date('%Y-%m-%d %H:%M')

	if xPlayer.job.name ~= 'cardealer' then
		print(('esx_nutzfahrzeugshop: %s attempted to add a sold vehicle to list!'):format(xPlayer.identifier))
		return
	end
...
ESX.RegisterServerCallback('esx_nutzfahrzeugshop:getSoldVehicles', function (source, cb)
...
RegisterServerEvent('esx_nutzfahrzeugshop:rentVehicle')
AddEventHandler('esx_nutzfahrzeugshop:rentVehicle', function (vehicle, plate, playerName, basePrice, rentPrice, target)
...
RegisterServerEvent('esx_nutzfahrzeugshop:getStockItem')
AddEventHandler('esx_nutzfahrzeugshop:getStockItem', function (itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_nutzfahrzeugshop:putStockItems')
AddEventHandler('esx_nutzfahrzeugshop:putStockItems', function (itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('esx_nutzfahrzeugshop:getCategories', function (source, cb)
	cb(Categories)
end)

ESX.RegisterServerCallback('esx_nutzfahrzeugshop:getVehicles', function (source, cb)
	cb(Vehicles)
end)

ESX.RegisterServerCallback('esx_nutzfahrzeugshop:buyVehicle', function (source, cb, vehicleModel)
...
ESX.RegisterServerCallback('esx_nutzfahrzeugshop:buyVehicleSociety', function (source, cb, society, vehicleModel)
	local vehicleData = nil
...
ESX.RegisterServerCallback('esx_nutzfahrzeugshop:getCommercialVehicles', function (source, cb)
...
RegisterServerEvent('esx_nutzfahrzeugshop:returnProvider')
AddEventHandler('esx_nutzfahrzeugshop:returnProvider', function(vehicleModel)
	local _source = source
	...
	MySQL.Async.fetchAll('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			print(('esx_nutzfahrzeugshop: %s attempted selling an invalid vehicle!'):format(GetPlayerIdentifiers(_source)[1]))
		end

	end)
end)

ESX.RegisterServerCallback('esx_nutzfahrzeugshop:getRentedVehicles', function (source, cb)
...
ESX.RegisterServerCallback('esx_nutzfahrzeugshop:giveBackVehicle', function (source, cb, plate)
...
ESX.RegisterServerCallback('esx_nutzfahrzeugshop:resellVehicle', function (source, cb, plate, model)
	local resellPrice = 0
	...
	if resellPrice == 0 then
		print(('esx_nutzfahrzeugshop: %s attempted to sell an unknown vehicle!'):format(GetPlayerIdentifiers(source)[1]))
		cb(false)
	else
		MySQL.Async.fetchAll('SELECT * FROM rented_vehicles WHERE plate = @plate', {
			['@plate'] = plate
					...
					if result[1] then -- does the owner match?
						...
						if vehicle.model == model then
							if vehicle.plate == plate then
								...
								print(('esx_nutzfahrzeugshop: %s attempted to sell an vehicle with plate mismatch!'):format(xPlayer.identifier))
								cb(false)
							end
						else
							print(('esx_nutzfahrzeugshop: %s attempted to sell an vehicle with model mismatch!'):format(xPlayer.identifier))
							cb(false)
						end
					else
...
ESX.RegisterServerCallback('esx_nutzfahrzeugshop:getStockItems', function (source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		cb(inventory.items)
	end)
end)

ESX.RegisterServerCallback('esx_nutzfahrzeugshop:getPlayerInventory', function (source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_nutzfahrzeugshop:isPlateTaken', function (source, cb, plate)
...
RegisterServerEvent('esx_nutzfahrzeugshop:setJobVehicleState')
AddEventHandler('esx_nutzfahrzeugshop:setJobVehicleState', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('esx_nutzfahrzeugshop: %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)
