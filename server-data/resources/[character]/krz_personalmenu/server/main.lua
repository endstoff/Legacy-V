...
AddEventHandler('KorioZ-PersonalMenu:Boss_promouvoirplayer', function(target)
	...
	if (targetXPlayer.job.grade == tonumber(getMaximumGrade(sourceXPlayer.job.name)) - 1) then
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Du brauchst die Erlaubnis der ~r~Regierung~w.')
	else
		if sourceXPlayer.job.grade_name == 'boss' and sourceXPlayer.job.name == targetXPlayer.job.name then
			...
			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Du hast ~g~' .. targetXPlayer.name .. '~w~ befördert.')
			TriggerClientEvent('esx:showNotification', target, 'Du wurdest von ~g~' .. sourceXPlayer.name .. '~w~ befördert.')
		else
			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Du hast keine ~r~Berechtigung~w~.')
		end
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_destituerplayer', function(target)
	...
	if (targetXPlayer.job.grade == 0) then
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Du kannst Ihn nicht weiter degradieren.')
	else
		if sourceXPlayer.job.grade_name == 'boss' and sourceXPlayer.job.name == targetXPlayer.job.name then
			targetXPlayer.setJob(targetXPlayer.job.name, tonumber(targetXPlayer.job.grade) - 1)

			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Du hast ~g~' .. targetXPlayer.name .. '~w~ degradiert.')
			TriggerClientEvent('esx:showNotification', target, 'Du wurdest von ~g~' .. sourceXPlayer.name .. '~w~. degradiert')
		else
			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Du hast keine ~r~Berechtigung~w~.')
		end
	end
...
AddEventHandler('KorioZ-PersonalMenu:Boss_recruterplayer', function(target, job, grade)
	...
	if sourceXPlayer.job.grade_name == 'boss' then
		...
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Du hast ~g~' .. targetXPlayer.name .. '~w~ eingestellt.')
		TriggerClientEvent('esx:showNotification', target, 'Du wurdest von ~g~' .. sourceXPlayer.name .. '~w~ eingestellt.')
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_virerplayer', function(target)
	...
	if sourceXPlayer.job.grade_name == 'boss' and sourceXPlayer.job.name == targetXPlayer.job.name then
		...
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Du hast ~r~' .. targetXPlayer.name .. '~w~ entlassen.')
		TriggerClientEvent('esx:showNotification', target, 'Du wurdest von ~r~' .. sourceXPlayer.name .. '~w~ entlassen.')
	else
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Du hast keine ~r~Berechtigung~w~.')
	end
end)
