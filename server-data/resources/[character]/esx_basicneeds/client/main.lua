...
AddEventHandler('esx_basicneeds:onCola', function(prop_name)
	if not IsAnimated then
		...
		Citizen.CreateThread(function()
			...
			local prop = CreateObject(GetHashKey(prop_name), x, y, z + 0.2, true, true, true)
			local boneIndex = GetPedBoneIndex(playerPed, 18905)
			AttachEntityToEntity(prop, playerPed, boneIndex, 0.12, 0.03, 0.003, 5.0, 100.0, 0.0, true, true, false, true, 3, true)
...
RegisterNetEvent('esx_basicneeds:onWein')
AddEventHandler('esx_basicneeds:onWein', function(prop_name)
	if not IsAnimated then
...
RegisterNetEvent('esx_basicneeds:onSprite')
AddEventHandler('esx_basicneeds:onSprite', function(prop_name)
	if not IsAnimated then
		prop_name = prop_name or 'ng_proc_sodacan_01b'
		IsAnimated = true
...
AddEventHandler('esx_basicneeds:onChips', function(prop_name)
	if not IsAnimated then
		prop_name = prop_name or 'ng_proc_food_chips01b'
		IsAnimated = true
...
RegisterNetEvent('esx_basicneeds:onMedic')
AddEventHandler('esx_basicneeds:onMedic', function(prop_name)
	if not IsAnimated then
		prop_name = prop_name or 'p_cs_shot_glass_2_s'
		IsAnimated = true
