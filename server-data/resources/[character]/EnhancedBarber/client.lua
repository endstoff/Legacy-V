...
Citizen.CreateThread(function()
	...
	while true do
		...
		if isInShop then
			...
			showInfobar('Drücke ~INPUT_CONTEXT~ um dir die Haare zu schneiden')
...
function generateMenu()
	...
	local beard = _menuPool:AddSubMenu(mainMenu, 'Barttyp und dichte')
	mainMenu.Items[3]:RightLabel(LastSkin['beard_1']-1 .. ' / ' .. beard1)
	local beardColor = _menuPool:AddSubMenu(mainMenu, 'Bartfarbe colour')
	mainMenu.Items[4]:RightLabel(LastSkin['beard_3']-1 .. ' / ' .. beardColorsAmount)

	local cosmetic = _menuPool:AddSubMenu(mainMenu, 'Kosmetisch')
	mainMenu.Items[5]:RightLabel('~y~→→→')
	local eyebrows = _menuPool:AddSubMenu(cosmetic, 'Augenbrauen')
	cosmetic.Items[1]:RightLabel(LastSkin['eyebrows_1']-1 .. ' / ' .. eyebrows1)
	local eyebrowsColor = _menuPool:AddSubMenu(cosmetic, 'Augenbrauenfarben')
	...
	local lipstickColor = _menuPool:AddSubMenu(cosmetic, 'Lippenstiftfarbe')
	...
	local confirm = NativeUI.CreateItem('~g~Bestätigen', 'Lass dir die Haare vom Barbier schneiden.')
	if Config.useESX then
