...
Queue.MaxPlayers = GetConvarInt("sv_maxclients", 65)
...
function Queue:DebugPrint(msg)
    if Queue.Debug then
        msg = "^3Warteschlange: ^0" .. tostring(msg) .. "^7"
        print(msg)
    end
end
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] wurde priorisiert und platziert %d/%d in der Warteschlange", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] wurde platziert %d/%d in der Warteschlange", tmp.name, ids[1], _pos, queueCount))
    end
    ...
    if type(id) == "table" then
        for _id, power in pairs(id) do
            if _id and type(_id) == "string" and power and type(power) == "number" then
                ...
                Queue:DebugPrint("Fehler beim Hinzufügen einer Prioritäts-ID, ungültige Daten übergeben")
                return false
            end
        end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "Ihr Beitritt wurde blockiert")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
            Queue:DebugPrint(string_format("%s[%s] wurde am Beitritt gehindert; Grund: %s", name, ids[1], reason))
            CancelEvent()
            return
        end
    ...
    if Queue:IsInQueue(ids) then
        ...
        Queue:DebugPrint(string_format("%s[%s] hat sich nach der Stornierung wieder in die Warteschlange eingereiht", name, ids[1]))
    else
        Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() then
        ...
        Queue:DebugPrint(name .. "[" .. ids[1] .. "] wird in den Server geladen")
    ...
    while true do
        ...
        if not data or not data.deferrals or not data.source or not pos then
            remove("[Warteschlange] Aus Warteschlange entfernt, Warteschlangendaten ungültig :(")
            Queue:DebugPrint(tostring(name .. "[" .. ids[1] .. "] aus der Warteschlange entfernt wurde, weil sie ungültige Daten hatten"))
            return
        end
        ...
        if data.timeout >= Config.QueueTimeOut and os_time() - connectTime > 5 then
            remove("[Warteschlange] Wegen Zeitüberschreitung entfernt")
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] wurde aus der Warteschlange entfernt, weil sie ihre Zeit überschritten hatten")
            return
        end
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Config.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] wurde aus der Verbindungswarteschlange entfernt, weil sie eine Zeitüberschreitung hatten")
            else
                i = i + 1
            end
        end
