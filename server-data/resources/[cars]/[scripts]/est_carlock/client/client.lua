...
Citizen.CreateThread(function()
  ...
  while true do
    Citizen.Wait(0)
	if (IsControlJustPressed(1, 303)) then --183 = Key U
		...
		notowned = 0
		if #cars == 0 then
			TriggerEvent('notifications', "#B90000", "Schlüssel", "Kein Fahrzeug in der Nähe zum abschließen.")
	--		ESX.ShowNotification("Keine Fahrzeuge in der Nähe zum abschließen.")
			...
			for i=1, #carstrie, 1 do
				...
				ESX.TriggerServerCallback('est_carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							exports['mythic_notify']:DoHudText('success', 'Fahrzeug abgeschlossen')
					--		TriggerEvent('notifications', "#B90000", "Schlüssel", "Fahrzeug abgeschlossen")
					--		ESX.ShowNotification('Du hast dein ~y~'..vehicleLabel..'~s~ ~r~abgeschlossen~s~.')
						...
						elseif lock == 2 then
							...
							exports['mythic_notify']:DoHudText('success', 'Fahrzeug aufgeschlossen')
					--		TriggerEvent('notifications', "#B90000", "Schlüssel", "Fahrzeug aufgeschlossen")
					...
					if notowned == #carstrie then
						exports['mythic_notify']:DoHudText('error', 'Kein Fahrzeug in der Nähe zum abschließen')
		--				TriggerEvent('notifications', "#B90000", "Schlüssel", "Kein Fahrzeug in der Nähe zum abschließen.")
		--				ESX.ShowNotification("Keine Fahrzeuge in der Nähe zum abschließen.")
					end	
				end, plate)
