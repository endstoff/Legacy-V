...
if Config.DisableExitingVehicle then
    ...
    loopwait = Config.SeatbeltLoopTimer
end
...
function SeatbeltCheck(ped, vehicle)
    if GetEntitySpeedVector(vehicle, true).y > -3.0 and t.speed_new > 5.0 then
        if not SeatBelt_on and (t.speed_old-t.speed_new) > 40.0 then
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if IsPedInAnyVehicle(ped, true) then
                ...
                t.speed_new = GetEntitySpeed(vehicle)*2.236936
                SeatbeltCheck(ped, vehicle)
                t.speed_old = t.speed_new
            else
                wait = 1000
                SeatBelt_on = false
            end
...
Citizen.CreateThread(function()
    while Authorised == false do Wait(1000) end
    while true do
        Wait(5)
        if IsPedInAnyVehicle(PlayerPedId()) then
            Wait(2000)
