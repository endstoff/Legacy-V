...
$(document).ready(function(){
    window.addEventListener('message', function( event ) {       
      if (event.data.action == 'open') {
        ...
        $.post('http://est_impound/enable-parkout', JSON.stringify({}));
    ...
    $( ".close" ).click(function() {
      ...
      $.post('http://est_impound/escape', JSON.stringify({}));
    }); 
    ...
    $( ".head-switch-out" ).click(function() {     
      ...
      $.post('http://est_impound/enable-parking', JSON.stringify({}));
...
function parkOut(plate) {
  if (toggle == false) {
    $('.container').css('display', 'none');
    $.post('http://est_impound/escape', JSON.stringify({}));
    $.post('http://est_impound/park-out', JSON.stringify({plate: plate}));
  } else if (toggle == true) {
    $('.container').css('display', 'none');
    $.post('http://est_impound/escape', JSON.stringify({}));
    $.post('http://est_impound/park-in', JSON.stringify({plate: plate}));
  }
}