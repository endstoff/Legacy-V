...
function OpenAlarmSelectMenu(plate,name,gotKey,alarm,model)	
	ESX.TriggerServerCallback("t1ger_keys:fetchCarPrice", function(vehPrice)
		
		local elements = {
			{ label = Lang['alarm_grade_0'] .. " (Kosten: $"..math.floor(vehPrice * (Config.RemoveAlarmPercentPrice/100))..")", value = 0, price = math.floor(vehPrice * (Config.RemoveAlarmPercentPrice/100)) },
			{ label = Lang['alarm_grade_1'].. " (Kosten: $"..math.floor(vehPrice * (Config.AlarmOnePercentPrice/100))..")", value = 1, price = math.floor(vehPrice * (Config.AlarmOnePercentPrice/100)) },
			{ label = Lang['alarm_grade_2'].. " (Kosten: $"..math.floor(vehPrice * (Config.AlarmTwoPercentPrice/100))..")", value = 2, price = math.floor(vehPrice * (Config.AlarmTwoPercentPrice/100)) },
		}
		...
		function(data, menu)
			TriggerServerEvent('t1ger_keys:updateCarAlarm', plate, data.current.value, model, data.current.price)
			menu.close()
			OpenAlarmShopMenu()
		end, function(data, menu)
			menu.close()
...
--		{ label = Lang['veh_neon_label'], value = "veh_neon_btn" },
	...
	function(data, menu)
		...
		if (data.current.value == 'hotwire') then
			HotwireCarFunction()
		end

		if (data.current.value == 'car_search') then
			SearchCarFunction()
		end
					...
					if not IsEntityDead(entity) and not IsPedDeadOrDying(entity, 1) then 
						if GetEntityPlayerIsFreeAimingAt(PlayerId(),entity) then 
							if Config.PedGivesKeyChance > keyDropChance then
								...
								TriggerServerEvent('t1ger_keys:stolenCarKeys', GetVehicleNumberPlateText(robCar):gsub("^%s*(.-)%s*$", "%1"))
							else
