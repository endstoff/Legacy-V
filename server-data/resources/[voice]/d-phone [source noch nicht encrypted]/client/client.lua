...
RegisterNetEvent("d-phone:client:hasnophone")
AddEventHandler("d-phone:client:hasnophone", function()
	hasphone = false
end)
...
RegisterNetEvent("d-phone:client:updatenumber")
AddEventHandler("d-phone:client:updatenumber", function(number)
	SendNUIMessage({
		updatenumber = true,
		number = number
	})
...
AddEventHandler('esx:onPlayerDeath', function(reason)
	...
	if incall == true and incallwith ~= nil then
		incall = false
		TriggerServerEvent("d-phone:server:endcall", GetPlayerServerId(PlayerId()), incallwith)
	elseif incall == true and incallwith == nil then
		TriggerServerEvent("d-phone:server:stopcall", GetPlayerServerId(PlayerId()), startcallnumber)
		incall = false
	elseif wantcall ~= nil then
		TriggerServerEvent("d-phone:server:declinecall", GetPlayerServerId(PlayerId()), wantcall)
		wantcall = nil
		wantcallnumber = nil
	end
...
AddEventHandler("d-phone:client:loadbusinessmessages", function(html, div)
    SendNUIMessage({
...
RegisterNUICallback("enablejobnumber", function(data, cb)
	TriggerServerEvent("d-phone:server:changejobnumber", source, data.job)
end)
