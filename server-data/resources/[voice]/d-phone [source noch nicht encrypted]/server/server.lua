...
AddEventHandler('d-phone:server:sendmessage', function(source, message, number)
    ...
    if isservicee == false then
        local Othersource = getOthersource(Otheridentifier)
...
AddEventHandler('d-phone:server:changewallpaper', function(source, url)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `phone_information` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
        if result[1] ~= nil then
            ...
            TriggerClientEvent("d-notification", _source, _U("changedbg"))
        else
            MySQL.Async.execute("INSERT INTO `phone_information` (`identifier`, `wallpaper`) VALUES (@identifier, @wallpaper)",
...
function getOthersource(identifier)
    local players = ESX.GetPlayers()
    local found = false
    for i=1, #players do
        local zPlayer = ESX.GetPlayerFromId(players[i])
        if zPlayer.identifier == identifier then
            found = true
            return zPlayer.source
        end
    end
end
...
AddEventHandler('d-phone:server:loadbusinessmessage', function(source, job, sender)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `phone_messages` WHERE `receiver` = @receiver AND `sender` = @sender", {["@receiver"] = job, ["@sender"] = sender}, function(message)
        MySQL.Async.fetchAll("SELECT * FROM `phone_messages` WHERE `receiver` = @receiver AND `sender` = @sender", {["@receiver"] = sender, ["@sender"] = job}, function(sendersql)
            ...
            div = div .. string.format('<div class="phone-businessapp-chat"></div><div class="phone-businessapp-delete"><button class="phone-businessapp-button bpb-accept" id="bpb-accept-%s" onclick="bpbaccept()">  <font id="pbbacceptfont">  </font>    </button>    <button class="phone-businessapp-button bpb-decline" id="bpb-decline-%s" onclick="bpbdecline()">        <font id="pbbacceptfont2">  </font>    </button></div><div class="phone-businessapp-input">    <input type="text" id="phone-business-input-message" class="phone-businessapp-inputt"></div><div class="phone-businessapp-place">    <i id="phone-businessapp-placeicon" style="position: relative;" class="fas fa-map-marker-alt"> </i>    <!-- <i id="phone-chat-contact" class="fas fa-address-book"> </i> -->    <i id="phone-businessapp-send" style="position: relative;" class="fas fa-paper-plane"> </i></div>', sender, sender)
            TriggerClientEvent("d-phone:client:loadbusinessmessages", _source, html, div)
        end)
    end)
end)
...
RegisterServerEvent('d-phone:server:changejobnumber')
AddEventHandler('d-phone:server:changejobnumber', function(source, job)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xPlayernumber = getNumberPhone(xPlayer.identifier)
    local jobnumber
    ...
    Wait(100)
    if jobnumber ~= "1" then
        local hasjobnumber = false
        if jobnumber == xPlayernumber then
            hasjobnumber = true
        end

        MySQL.Async.fetchAll("SELECT * FROM `users` WHERE `phone_number` = @phone_number", {["@phone_number"] = jobnumber}, function(db)
            if db[1] == nil then
                MySQL.Async.fetchAll("SELECT * FROM `users` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
                    if result[1].phone_number2 ~= "1" then
                        TriggerClientEvent("d-notification", _source, _U("normalnumber"))
                        TriggerClientEvent("d-phone:client:updatenumber", _source, result[1].phone_number2)
                        MySQL.Async.execute("UPDATE users SET phone_number = @phone_number, phone_number2 = @phone_number2 WHERE identifier = @identifier", {["@phone_number"] = result[1].phone_number2, ["@phone_number2"] = "1", ["@identifier"] = xPlayer.identifier}, function()end)
                    else
                        TriggerClientEvent("d-notification", _source, _U("changedjobnumber"))
                        TriggerClientEvent("d-phone:client:updatenumber", _source, jobnumber)
                        MySQL.Async.execute("UPDATE users SET phone_number = @phone_number, phone_number2 = @phone_number2 WHERE identifier = @identifier", {["@phone_number"] = jobnumber, ["@phone_number2"] = xPlayernumber, ["@identifier"] = xPlayer.identifier}, function()end)
                    end
                end)
            else
                if db[1].identifier ~= xPlayer.identifier then
                    TriggerClientEvent("d-notification", _source, _U("sbusingjobnumber"))
                else
                    TriggerClientEvent("d-notification", _source, _U("normalnumber"))
                    TriggerClientEvent("d-phone:client:updatenumber", _source, db[1].phone_number2)
                    MySQL.Async.execute("UPDATE users SET phone_number = @phone_number, phone_number2 = @phone_number2 WHERE identifier = @identifier", {["@phone_number"] = db[1].phone_number2, ["@phone_number2"] = "1", ["@identifier"] = xPlayer.identifier}, function()end)
                end
            end
        end)
    else
        TriggerClientEvent("d-notification", _source, _U("yourjobdonthaveanumber"))
    end
end)
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
        ...
        if xPlayer ~= nil and xPlayer.job ~= nil  then
            MySQL.Async.fetchAll("SELECT * FROM `jobs`", {}, function(result)
                for i,v in ipairs(result) do              
                    if v.label == xPlayer.job.label then
                        jobnumber = v.number
                    end
                end
            end)
